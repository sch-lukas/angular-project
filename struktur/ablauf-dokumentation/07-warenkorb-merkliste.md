# 07 - Warenkorb und Merkliste

> **Szenario:** Ein Benutzer fügt Bücher zum Warenkorb oder zur Merkliste hinzu, ohne eingeloggt zu sein.

---

## ⚡ Besonderheit: Rein Frontend-basiert

**Dieser Flow hat KEINE Backend-Kommunikation!**

- Warenkorb und Merkliste werden komplett im Browser gespeichert
- Verwendung von `localStorage` für Persistenz
- Kein Login erforderlich
- Daten bleiben auch nach Browser-Neustart erhalten

---

## 🔄 Ablauf-Diagramm: Zum Warenkorb hinzufügen

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ 1. AUSGANGSPUNKT: Detailseite eines Buchs                                            │
│                                                                                      │
│    ┌─────────────────────────────────────────────────────────────────────────────┐  │
│    │  ╔═══════════════════════════════════════════════════════════════════════╗  │  │
│    │  ║  📘 Angular Masterclass                                               ║  │  │
│    │  ║  Von Anfänger zum Profi                                               ║  │  │
│    │  ║                                                                       ║  │  │
│    │  ║  Preis: €39.95                                                        ║  │  │
│    │  ║  Rating: ★★★★★                                                        ║  │  │
│    │  ║                                                                       ║  │  │
│    │  ║  [🛒 In den Warenkorb]  [❤️ Auf Merkliste]                            ║  │  │
│    │  ╚═══════════════════════════════════════════════════════════════════════╝  │  │
│    └─────────────────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼ User klickt "In den Warenkorb"
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ 2. DETAIL-COMPONENT: Event-Handler                                                   │
│                                                                                      │
│    detail.component.ts:                                                              │
│    ┌─────────────────────────────────────────────────────────────────────────────┐  │
│    │ addToCart(): void {                                                          │  │
│    │     if (!this.buch) return;                                                  │  │
│    │                                                                              │  │
│    │     this.cartService.addToCart(this.buch);                                   │  │
│    │                                                                              │  │
│    │     // Feedback für User                                                     │  │
│    │     this.showToast('Zum Warenkorb hinzugefügt! 🛒');                         │  │
│    │ }                                                                            │  │
│    └─────────────────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ 3. CART-SERVICE: Buch zum Warenkorb hinzufügen                                       │
│                                                                                      │
│    cart.service.ts:                                                                  │
│    ┌─────────────────────────────────────────────────────────────────────────────┐  │
│    │ @Injectable({ providedIn: 'root' })                                          │  │
│    │ export class CartService {                                                   │  │
│    │     private readonly STORAGE_KEY = 'bookshop_cart';                          │  │
│    │                                                                              │  │
│    │     // BehaviorSubject für reaktive Updates                                  │  │
│    │     private cartItems$ = new BehaviorSubject<CartItem[]>(                    │  │
│    │         this.loadFromStorage()                                               │  │
│    │     );                                                                       │  │
│    │                                                                              │  │
│    │     addToCart(buch: Buch): void {                                            │  │
│    │         const current = this.cartItems$.getValue();                          │  │
│    │                                                                              │  │
│    │         // Prüfen ob Buch bereits im Warenkorb                               │  │
│    │         const existing = current.find(item => item.buch.id === buch.id);     │  │
│    │         if (existing) {                                                      │  │
│    │             existing.quantity++;  // Menge erhöhen                           │  │
│    │         } else {                                                             │  │
│    │             current.push({ buch, quantity: 1 });  // Neu hinzufügen          │  │
│    │         }                                                                    │  │
│    │                                                                              │  │
│    │         // In localStorage speichern                                         │  │
│    │         this.saveToStorage(current);                                         │  │
│    │                                                                              │  │
│    │         // Alle Subscriber benachrichtigen                                   │  │
│    │         this.cartItems$.next(current);                                       │  │
│    │     }                                                                        │  │
│    │ }                                                                            │  │
│    └─────────────────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ 4. LOCAL-STORAGE: Daten persistieren                                                 │
│                                                                                      │
│    cart.service.ts:                                                                  │
│    ┌─────────────────────────────────────────────────────────────────────────────┐  │
│    │ private saveToStorage(items: CartItem[]): void {                             │  │
│    │     // Nur relevante Daten speichern (nicht das ganze Buch-Objekt)           │  │
│    │     const toStore = items.map(item => ({                                     │  │
│    │         buchId: item.buch.id,                                                │  │
│    │         titel: item.buch.titel?.titel,                                       │  │
│    │         preis: item.buch.preis,                                              │  │
│    │         quantity: item.quantity                                              │  │
│    │     }));                                                                     │  │
│    │                                                                              │  │
│    │     localStorage.setItem(                                                    │  │
│    │         this.STORAGE_KEY,                                                    │  │
│    │         JSON.stringify(toStore)                                              │  │
│    │     );                                                                       │  │
│    │ }                                                                            │  │
│    │                                                                              │  │
│    │ private loadFromStorage(): CartItem[] {                                      │  │
│    │     const stored = localStorage.getItem(this.STORAGE_KEY);                   │  │
│    │     if (!stored) return [];                                                  │  │
│    │                                                                              │  │
│    │     try {                                                                    │  │
│    │         return JSON.parse(stored);                                           │  │
│    │     } catch {                                                                │  │
│    │         return [];                                                           │  │
│    │     }                                                                        │  │
│    │ }                                                                            │  │
│    └─────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                      │
│    localStorage Inhalt:                                                              │
│    ┌─────────────────────────────────────────────────────────────────────────────┐  │
│    │ Key: "bookshop_cart"                                                         │  │
│    │ Value: [                                                                     │  │
│    │     {                                                                        │  │
│    │         "buchId": 1,                                                         │  │
│    │         "titel": "Angular Masterclass",                                      │  │
│    │         "preis": 39.95,                                                      │  │
│    │         "quantity": 1                                                        │  │
│    │     }                                                                        │  │
│    │ ]                                                                            │  │
│    └─────────────────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ 5. NAVBAR: Warenkorb-Badge aktualisieren                                             │
│                                                                                      │
│    navbar.component.ts:                                                              │
│    ┌─────────────────────────────────────────────────────────────────────────────┐  │
│    │ // Im Template (oder als Computed Signal)                                    │  │
│    │ cartCount$ = this.cartService.getItemCount();                                │  │
│    │                                                                              │  │
│    │ // Template:                                                                 │  │
│    │ <a routerLink="/cart" class="nav-link">                                      │  │
│    │     🛒 Warenkorb                                                             │  │
│    │     <span *ngIf="cartCount$ | async as count"                                │  │
│    │           class="badge bg-danger">                                           │  │
│    │         {{ count }}                                                          │  │
│    │     </span>                                                                  │  │
│    │ </a>                                                                         │  │
│    └─────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                      │
│    Visuelles Ergebnis:                                                               │
│    ┌─────────────────────────────────────────────────────────────────────────────┐  │
│    │   [Home] [Suche] [Anlegen]                   🛒 Warenkorb (1)  [Login]       │  │
│    │                                                      ▲                       │  │
│    │                                            Badge zeigt Anzahl                │  │
│    └─────────────────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────────────┘
```

---

## 🔄 Ablauf-Diagramm: Merkliste

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│ MERKLISTE: Gleicher Flow wie Warenkorb                                               │
│                                                                                      │
│    wishlist.service.ts:                                                              │
│    ┌─────────────────────────────────────────────────────────────────────────────┐  │
│    │ @Injectable({ providedIn: 'root' })                                          │  │
│    │ export class WishlistService {                                               │  │
│    │     private readonly STORAGE_KEY = 'bookshop_wishlist';                      │  │
│    │                                                                              │  │
│    │     // Nur Buch-IDs speichern (keine Mengen)                                 │  │
│    │     private wishlistIds$ = new BehaviorSubject<number[]>(                    │  │
│    │         this.loadFromStorage()                                               │  │
│    │     );                                                                       │  │
│    │                                                                              │  │
│    │     toggleWishlist(buchId: number): void {                                   │  │
│    │         const current = this.wishlistIds$.getValue();                        │  │
│    │         const index = current.indexOf(buchId);                               │  │
│    │                                                                              │  │
│    │         if (index > -1) {                                                    │  │
│    │             current.splice(index, 1);  // Entfernen                          │  │
│    │         } else {                                                             │  │
│    │             current.push(buchId);  // Hinzufügen                             │  │
│    │         }                                                                    │  │
│    │                                                                              │  │
│    │         this.saveToStorage(current);                                         │  │
│    │         this.wishlistIds$.next([...current]);                                │  │
│    │     }                                                                        │  │
│    │                                                                              │  │
│    │     isInWishlist(buchId: number): Observable<boolean> {                      │  │
│    │         return this.wishlistIds$.pipe(                                       │  │
│    │             map(ids => ids.includes(buchId))                                 │  │
│    │         );                                                                   │  │
│    │     }                                                                        │  │
│    │ }                                                                            │  │
│    └─────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                      │
│    Unterschied zum Warenkorb:                                                        │
│    - Kein "quantity" (ein Buch kann nur einmal auf der Liste sein)                   │
│    - Toggle-Funktion (erneut klicken = entfernen)                                    │  │
│    - Herz-Icon ändert sich: ♡ → ❤️                                                  │
└─────────────────────────────────────────────────────────────────────────────────────┘
```

---

## 📁 Beteiligte Dateien

| Datei                                    | Rolle                                    |
| ---------------------------------------- | ---------------------------------------- |
| `frontend/src/app/cart.service.ts`       | Warenkorb-Logik, localStorage            |
| `frontend/src/app/wishlist.service.ts`   | Merkliste-Logik, localStorage            |
| `frontend/src/app/detail.component.ts`   | Buttons "In Warenkorb" / "Auf Merkliste" |
| `frontend/src/app/navbar.component.ts`   | Badge mit Anzahl                         |
| `frontend/src/app/cart.component.ts`     | Warenkorb-Seite                          |
| `frontend/src/app/wishlist.component.ts` | Merkliste-Seite                          |

---

## 🔍 Wichtige Konzepte

### 1. BehaviorSubject für reaktive State-Updates

```typescript
// BehaviorSubject: Observable mit initialem Wert
private cartItems$ = new BehaviorSubject<CartItem[]>([]);

// Aktuellen Wert lesen (synchron)
const current = this.cartItems$.getValue();

// Neuen Wert setzen → alle Subscriber werden benachrichtigt
this.cartItems$.next(newItems);

// Als Observable für Components
getItems(): Observable<CartItem[]> {
    return this.cartItems$.asObservable();
}
```

### 2. localStorage API

```typescript
// Speichern (nur Strings erlaubt!)
localStorage.setItem('key', JSON.stringify(data));

// Laden
const data = JSON.parse(localStorage.getItem('key') || '[]');

// Löschen
localStorage.removeItem('key');

// Alles löschen
localStorage.clear();
```

### 3. Toggle-Pattern für Merkliste

```typescript
// Template
<button (click)="toggleWishlist(buch.id)">
    {{ (isInWishlist$ | async) ? '❤️' : '♡' }}
</button>

// Component
isInWishlist$ = this.wishlistService.isInWishlist(this.buch.id);
```

---

## 🗺️ Architektur-Übersicht

```
┌─────────────────────────────────────────────────────────────────┐
│                         FRONTEND                                 │
│                                                                  │
│  ┌─────────────┐   ┌────────────────┐   ┌──────────────────┐   │
│  │   Detail    │   │    Navbar      │   │     Cart         │   │
│  │  Component  │   │   Component    │   │   Component      │   │
│  └──────┬──────┘   └───────┬────────┘   └────────┬─────────┘   │
│         │                  │                      │              │
│         │   ┌──────────────┴──────────────┐      │              │
│         │   │                             │      │              │
│         ▼   ▼                             ▼      ▼              │
│  ┌────────────────────────────────────────────────────┐        │
│  │              CartService / WishlistService          │        │
│  │                                                     │        │
│  │  - BehaviorSubject<CartItem[]>                      │        │
│  │  - addToCart() / removeFromCart()                   │        │
│  │  - getTotal() / getItemCount()                      │        │
│  └────────────────────────┬───────────────────────────┘        │
│                           │                                     │
│                           ▼                                     │
│  ┌────────────────────────────────────────────────────┐        │
│  │                   localStorage                       │        │
│  │                                                     │        │
│  │  bookshop_cart: [{"buchId": 1, "quantity": 2}, ...] │        │
│  │  bookshop_wishlist: [1, 5, 12]                      │        │
│  └────────────────────────────────────────────────────┘        │
│                                                                  │
│                    ❌ KEIN Backend-Call!                        │
└─────────────────────────────────────────────────────────────────┘
```

---

## 🔢 Warenkorb-Berechnungen

```typescript
// cart.service.ts

// Gesamtanzahl Artikel
getItemCount(): Observable<number> {
    return this.cartItems$.pipe(
        map(items => items.reduce((sum, item) => sum + item.quantity, 0))
    );
}

// Gesamtpreis
getTotal(): Observable<number> {
    return this.cartItems$.pipe(
        map(items => items.reduce(
            (sum, item) => sum + (item.buch.preis * item.quantity),
            0
        ))
    );
}

// Preis mit Rabatt
getTotalWithDiscount(): Observable<number> {
    return this.cartItems$.pipe(
        map(items => items.reduce((sum, item) => {
            const rabatt = item.buch.rabatt || 0;
            const preis = item.buch.preis * (1 - rabatt);
            return sum + (preis * item.quantity);
        }, 0))
    );
}
```

---

## 💡 Warum kein Backend?

| Aspekt            | Begründung                           |
| ----------------- | ------------------------------------ |
| **Einfachheit**   | Kein Server-Roundtrip nötig          |
| **Offline-fähig** | Funktioniert ohne Internetverbindung |
| **Schnelligkeit** | Sofortige UI-Updates                 |
| **Kein Account**  | User muss nicht eingeloggt sein      |
| **Datenschutz**   | Daten bleiben beim User              |

### Nachteil:

- Warenkorb ist **nicht synchron** zwischen Geräten
- Bei Löschen von Browser-Daten → Warenkorb weg

---

_Ende der Ablauf-Dokumentation_
