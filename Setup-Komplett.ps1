...
function Ensure-TlsCertificates {
    ...
    if ((Test-Path $keyFile) -and (Test-Path $certFile)) {
        Write-Info "TLS Zertifikate bereits vorhanden"
        return
    }
    ...
    Write-Success "TLS Zertifikate mit Docker/openssl erstellt"
}
...
$keycloakAdminPassword = Read-Host "Keycloak Admin Passwort festlegen"
if ([string]::IsNullOrWhiteSpace($keycloakAdminPassword)) {
    $keycloakAdminPassword = "CHANGE_ME_DEV_PASSWORD"
}

$keycloakAppPassword = Read-Host "Keycloak App Passwort (admin/user) festlegen"
if ([string]::IsNullOrWhiteSpace($keycloakAppPassword)) {
    $keycloakAppPassword = "CHANGE_ME_DEV_PASSWORD"
}
...
KEYCLOAK_APP_PASSWORD=$keycloakAppPassword
...
if (-not (Test-Path $envPath) -or $Force) {
    ...
    $existingClientSecret = Get-EnvValue -Name "CLIENT_SECRET" -EnvPath $envPath
    if ([string]::IsNullOrWhiteSpace($existingClientSecret) -or $existingClientSecret -like "__SET_*" -or $existingClientSecret -like "CHANGE_ME*") {
    ...
    $existingAdminPw = Get-EnvValue -Name "KEYCLOAK_ADMIN_PASSWORD" -EnvPath $envPath
    if ([string]::IsNullOrWhiteSpace($existingAdminPw) -or $existingAdminPw -like "CHANGE_ME*") {
        Set-EnvValue -Name "KEYCLOAK_ADMIN_PASSWORD" -Value $keycloakAdminPassword -EnvPath $envPath
        $needsUpdate = $true
    }

    $existingAppPw = Get-EnvValue -Name "KEYCLOAK_APP_PASSWORD" -EnvPath $envPath
    if ([string]::IsNullOrWhiteSpace($existingAppPw) -or $existingAppPw -like "CHANGE_ME*") {
        Set-EnvValue -Name "KEYCLOAK_APP_PASSWORD" -Value $keycloakAppPassword -EnvPath $envPath
        $needsUpdate = $true
    }
...
Write-Host "║  │    Keycloak:   http://localhost:8880 (admin/<aus .env>)         │  ║" -ForegroundColor Green
...
Write-Host "║  │    Passwort:   siehe KEYCLOAK_APP_PASSWORD in .env             │  ║" -ForegroundColor Green
Write-Host "║  └─────────────────────────────────────────────────────────────────┘  ║" -ForegroundColor Green
Write-Host "║                                                                       ║" -ForegroundColor Green
Write-Host "║  ┌─────────────────────────────────────────────────────────────────┐  ║" -ForegroundColor Green
