/**
 * Script zum Generieren von Cover-Bildern für alle Bücher in der Datenbank
 * Speichert die Bilder in frontend/src/assets/covers/ mit dem Dateinamen: {id}.jpg
 */
 ...
 */
function generateCover(buch, titel) {
    const width = 400;
    const height = 600;
    ...
    const titleText = titel?.titel || 'Unbekannter Titel';
    ...
    if (titel?.untertitel) {
        ctx.font = '20px Arial';
        y += 40;
        ctx.fillText(titel.untertitel, width / 2, y);
    }
...
async function main() {
    try {
        console.log('🔍 Lade alle Bücher aus der Datenbank...\n');
        ...
        for (const buch of buecher) {
            try {
                const titelText = buch.titel?.titel || 'Unbekannt';
                console.log(
                    `🎨 Generiere Cover für Buch ${buch.id}: "${titelText}"`,
                );

                const canvas = generateCover(buch, buch.titel);
